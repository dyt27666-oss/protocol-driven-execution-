...
<p>把"用户全程盯流程"降级为"只在 chips 出现时排版反馈" · Stage 0 contract → 7-stage 循环 → Codex 独立 review</p>
...
<th align="left">✨ After（用 step-to-step-chips）</th>
</tr>
<tr>
<td>
...
- 🎯 **chips 反馈机制** — 架构决策用 AskUserQuestion 结构化选项，2-3 秒判断完
...
│   ├── chips-design.md                     # AskUserQuestion 设计模式
...
step-to-step-chips 是 **agent orchestration wrapper**，**不替代**团队既有质量流程：
...
不适用的场景：**纯 LLM API 调用**（无 TaskList / AskUserQuestion 工具）、**完全脚本化 pipeline**（无交互式 chips 反馈）。
...
Claude Code 全支持。Codex CLI 通过 `spawn_agent` + 自定义 tool 可部分实现。**纯 LLM API** 无原生 task tracker，需要自行实现。
...
| Codex CLI | 部分支持（需自实现 task tracker） |
| 其他 agent | 需平台具备 task tracker + 结构化决策 prompt |
| 外部依赖 | 无 |
...
- 🐛 [报告 Bug](https://github.com/dyt27666-oss/step-to-step-chips/issues/new)
- 💡 [功能建议](https://github.com/dyt27666-oss/step-to-step-chips/issues/new)
- 🌍 翻译到其他语言
